...
;; point, `ediprolog-dwim' does the "appropriate" thing: If there's a
;; query running, push mark and go back to it. Otherwise, consult the
;; buffer and if point is on a query, sends the query to a Prolog process,
;; and you interact with the process in the current buffer as on a terminal.
;; You can press \\[keyboard-quit] to detach from the running query.
...
;;   C-1 F10       consult buffer, using a new process
;;   C-2 F10       equivalent to `ediprolog-remove-interactions'
;;   C-u F10       consult, using a new process, and run query

;; Tested with SWI-Prolog 7.3.21 + Emacs 22.1, 23.4, 24.5, 25.1, 26.0 and 26.1
...
(defcustom ediprolog-use-minibuffer t
  "Non-nil to indicate the minibuffer should be used, false to
indicate a custom consult buffer."
  :group 'ediprolog
  :type '(choice (const :tag "Use the minibuffer" t)
                 (const :tag "Use the custom buffer" nil)))

(defcustom ediprolog-make-consult-buffer-dedicated nil
  "Non-nil to indicate the minibuffer should be used, false to
indicate a custom consult buffer."
  :group 'ediprolog
  :type '(choice (const :tag "Make the consult buffer be dedicated" t)
...
(defun ediprolog-show-consult-output (str)
  (if (and ediprolog-use-minibuffer
	   (not (or (string-match "^[\t ]*\\(?:%.*\\)?\\'" str)
		    (let ((success "true.")) 	; TODO If adding support for other engines, change
  ...
  ;; Unless the output is a prefix of success (i.e., consulted without errors),
  ;; or still an incomplete line that starts with a comment character.
  ;; If the custom variable `ediprolog-use-minibuffer' is non-nil,
  ;; show output in minibuffer. Otherwise show consult output in a
  ;; separate window.
  (unless (or (string-match "^[\t ]*\\(?:%.*\\)?\\'" str)
              (let ((success "true.")) 	; TODO If adding support for other engines, change
                (and (<= (length str) (length success))
                     (string= str (substring success 0 (length str)))))
	      (eq ediprolog-use-minibuffer t))
    (setq ediprolog-consult-window (display-buffer ediprolog-consult-buffer))
    (when (eq ediprolog-make-consult-buffer-dedicated t)
...
If there's a querry running, push the mark and go to it.
Otherwise, call `ediprolog-consult', and, if invoked on a line
starting with `:-' or `?-', possibly preceded by `%' and whitespace,
call `ediprolog-query' with the query as argument.
With a universal argument (\\keymap & \\[universal-argument]),
consults the buffer in a new process and interacts with the query.
\\[keyboard-quit] detaches from a running querry.

ARG changes the meaning:
With prefix 0, equivalent to `ediprolog-kill-prolog'.
With prefix 1, equivalent to `ediprolog-consult' with a new Prolog process.
With prefix 2, equivalent to `ediprolog-remove-interactions'."
  (interactive "P")
  (cond
   ((null arg) ;; Regular call
    ;; If there are more solutions, go to them
    (if (and ediprolog-interrupted (ediprolog-more-solutions))
    	(progn
	  (push-mark nil t nil)
	  (message "Mark set. Returning to running query")
	  (ediprolog-toplevel))
      ;; Otherwise, consult the buffer and run a querry
      (ediprolog-consult)
      (ediprolog-query))
    )
   ;; One universal argument, start new process
   ((equal arg '(4)) (ediprolog-consult t) (ediprolog-query))
   ((eq arg 0)
    (unless (ediprolog-running)
      (error "No Prolog process running"))
    (ediprolog-kill-prolog)
    (message "Prolog process killed."))
   ((eq arg 1) (ediprolog-consult t))
   ((eq arg 2) (ediprolog-remove-interactions))
...
(defun ediprolog-query ()
  ...
  (when (and (not (and transient-mark-mode mark-active))
    ...
    (let* ((from (goto-char (match-end 0)))
      ...
      (end-of-line)
      (newline)
      (insert ediprolog-indent-prefix ediprolog-prefix)
      (ediprolog-interact
       (format "%s\n" (mapconcat #'identity
...
operates on the region."
  ...
  (let ((line (with-current-buffer ediprolog-temp-buffer
    ...
    (unless (and line ediprolog-use-minibuffer)
      (message "%s consulted." (if (and transient-mark-mode mark-active)
				   "Region" "Buffer")))
    (when line
      (when (and transient-mark-mode mark-active (fboundp 'line-number-at-pos))
        (setq line (+ (line-number-at-pos (region-beginning)) line -1)))
        ;; (goto-line line)
      (push-mark
       nil ;; Current position
       ediprolog-use-minibuffer ;; If using the minibuffer, don't show the message
                                ;;saying mark was set, as that obstructs the error message
       nil ;; Don't activate the mark
       )
      (goto-char (point-min))
      (forward-line (1- line)))))
